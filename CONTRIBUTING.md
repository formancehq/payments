...
# THINGS TO NOTE WHILE WORKING IN THE CODEBASE
...
When representing additional values, you need to use metadata to do this. First, all the metadata must be namespaced according to the connector you are working with. The metadata properties must be prefixed with the column namespace. For instance, given a connector named `stripe`, you have to create a namespace that looks like this: `com.stripe.spec/`. This will be the prefix for all metadata created under the `stripe` connector.
