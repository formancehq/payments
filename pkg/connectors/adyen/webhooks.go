...
type supportedWebhook struct {
	urlPath string
	fn      func(context.Context, connector.TranslateWebhookRequest) (connector.TranslateWebhookResponse, error)
}
...
func (p *Plugin) createWebhooks(ctx context.Context, req connector.CreateWebhooksRequest) ([]connector.PSPWebhookConfig, error) {
	configs := make([]connector.PSPWebhookConfig, 0, 1)
...
func (p *Plugin) verifyWebhook(_ context.Context, req connector.VerifyWebhookRequest) (connector.VerifyWebhookResponse, error) {
	if !p.client.VerifyWebhookBasicAuth(req.Webhook.BasicAuth) {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("invalid basic auth: %w", connector.ErrWebhookVerification)
	}
	...
	for _, item := range *webhooks.NotificationItems {
		if !p.client.VerifyWebhookHMAC(item, req.Config.Metadata[webhookHMACMetadataKey]) {
			return connector.VerifyWebhookResponse{}, fmt.Errorf("invalid HMAC: %w", connector.ErrWebhookVerification)
		}
	}
...
func (p *Plugin) translateStandardWebhook(ctx context.Context, req connector.TranslateWebhookRequest) (connector.TranslateWebhookResponse, error) {
	webhooks, err := p.client.TranslateWebhook(string(req.Webhook.Body))
	if err != nil {
		return connector.TranslateWebhookResponse{}, err
	}

	responses := make([]connector.WebhookResponse, 0, len(*webhooks.NotificationItems))
	for _, item := range *webhooks.NotificationItems {
		var payment *connector.PSPPayment
		var err error
		...
		if payment != nil {
			responses = append(responses, connector.WebhookResponse{
				Payment: payment,
			})
		}
	}
...
func (p *Plugin) handleAuthorisation(
	...
	status := connector.PAYMENT_STATUS_AUTHORISATION
	if item.Success == "false" {
		status = connector.PAYMENT_STATUS_FAILED
	}
...
func (p *Plugin) handleAuthorisationAdjustment(
	...
	payment := connector.PSPPayment{
		...
		Status:                      connector.PAYMENT_STATUS_AMOUNT_ADJUSTMENT,
		DestinationAccountReference: pointer.For(item.MerchantAccountCode),
		Raw:                         raw,
	}
	return &payment, nil
...
func (p *Plugin) handleCancellation(
	...
	payment := connector.PSPPayment{
		...
		Asset:                       currency.FormatAsset(supportedCurrenciesWithDecimal, item.Amount.Currency),
		Scheme:                      parseScheme(item.PaymentMethod),
		Status:                      connector.PAYMENT_STATUS_CANCELLED,
		DestinationAccountReference: pointer.For(item.MerchantAccountCode),
		Raw:                         raw,
	}
...
func (p *Plugin) handleCapture(
	...
	payment := connector.PSPPayment{
		...
		Asset:                       currency.FormatAsset(supportedCurrenciesWithDecimal, item.Amount.Currency),
		Scheme:                      connector.PAYMENT_SCHEME_OTHER,
		Status:                      connector.PAYMENT_STATUS_CAPTURE,
		DestinationAccountReference: pointer.For(item.MerchantAccountCode),
		Raw:                         raw,
	}
	return &payment, nil
...
func (p *Plugin) handleCaptureFailed(
	...
	payment := connector.PSPPayment{
		...
		Asset:                       currency.FormatAsset(supportedCurrenciesWithDecimal, item.Amount.Currency),
		Scheme:                      connector.PAYMENT_SCHEME_OTHER,
		Status:                      connector.PAYMENT_STATUS_CAPTURE_FAILED,
		DestinationAccountReference: pointer.For(item.MerchantAccountCode),
		Raw:                         raw,
	}
...
func (p *Plugin) handleRefundedReversed(
	...
	payment := connector.PSPPayment{
		...
		Asset:                       currency.FormatAsset(supportedCurrenciesWithDecimal, item.Amount.Currency),
		Scheme:                      connector.PAYMENT_SCHEME_OTHER,
		Status:                      connector.PAYMENT_STATUS_REFUND_REVERSED,
		DestinationAccountReference: pointer.For(item.MerchantAccountCode),
		Raw:                         raw,
	}
...
func (p *Plugin) handlePayoutExpire(
	...
	payment := connector.PSPPayment{
		...
		Asset:                  currency.FormatAsset(supportedCurrenciesWithDecimal, item.Amount.Currency),
		Scheme:                 connector.PAYMENT_SCHEME_OTHER,
		Status:                 connector.PAYMENT_STATUS_EXPIRED,
		SourceAccountReference: pointer.For(item.MerchantAccountCode),
		Raw:                    raw,
	}
...
func parseScheme(scheme string) connector.PaymentScheme {
	switch {
	case strings.HasPrefix(scheme, "visa"):
		return connector.PAYMENT_SCHEME_CARD_VISA
	case strings.HasPrefix(scheme, "electron"):
		return connector.PAYMENT_SCHEME_CARD_VISA
	case strings.HasPrefix(scheme, "amex"):
		return connector.PAYMENT_SCHEME_CARD_AMEX
	case strings.HasPrefix(scheme, "alipay"):
		return connector.PAYMENT_SCHEME_CARD_ALIPAY
	case strings.HasPrefix(scheme, "cup"):
		return connector.PAYMENT_SCHEME_CARD_CUP
	case strings.HasPrefix(scheme, "discover"):
		return connector.PAYMENT_SCHEME_CARD_DISCOVER
	case strings.HasPrefix(scheme, "doku"):
		return connector.PAYMENT_SCHEME_DOKU
	case strings.HasPrefix(scheme, "dragonpay"):
		return connector.PAYMENT_SCHEME_DRAGON_PAY
	case strings.HasPrefix(scheme, "jcb"):
		return connector.PAYMENT_SCHEME_CARD_JCB
	case strings.HasPrefix(scheme, "maestro"):
		return connector.PAYMENT_SCHEME_MAESTRO
	case strings.HasPrefix(scheme, "mc"):
		return connector.PAYMENT_SCHEME_CARD_MASTERCARD
	case strings.HasPrefix(scheme, "molpay"):
		return connector.PAYMENT_SCHEME_MOL_PAY
	case strings.HasPrefix(scheme, "diners"):
		return connector.PAYMENT_SCHEME_CARD_DINERS
	default:
		return connector.PAYMENT_SCHEME_OTHER
	}
}
