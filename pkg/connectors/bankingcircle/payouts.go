...
func (p *Plugin) validatePayoutRequest(pi connector.PSPPaymentInitiation) error {
	...
	if pi.DestinationAccount.Metadata[connector.AccountAccountNumberMetadataKey] == "" &&
		pi.DestinationAccount.Metadata[connector.AccountIBANMetadataKey] == "" {
		return connector.NewWrappedError(
...
func (p *Plugin) createPayout(ctx context.Context, pi connector.PSPPaymentInitiation) (*connector.PSPPayment, error) {
	...
	account := pi.DestinationAccount.Metadata[connector.AccountAccountNumberMetadataKey]
	if account == "" {
		account = pi.DestinationAccount.Metadata[connector.AccountIBANMetadataKey]
	}
	...
	resp, err := p.client.InitiateTransferOrPayouts(ctx, &client.PaymentRequest{
		...
		CreditorAccount: &client.PaymentAccount{
			Account:              account,
			FinancialInstitution: pi.DestinationAccount.Metadata[connector.AccountSwiftBicCodeMetadataKey],
			Country:              pi.DestinationAccount.Metadata[connector.AccountBankAccountCountryMetadataKey],
		},
