...
var _ = Describe("Moneycorp *Plugin Payments - check types and minor conversion", func() {
	...
	Context("fetch next Payments", func() {
		...
		It("fetches payments", func(ctx SpecContext) {
			...
			Expect(res.Payments[0].Type).To(Equal(connector.PAYMENT_TYPE_TRANSFER))
			...
			Expect(res.Payments[1].Type).To(Equal(connector.PAYMENT_TYPE_PAYOUT))
			...
			Expect(res.Payments[2].Type).To(Equal(connector.PAYMENT_TYPE_PAYOUT))
