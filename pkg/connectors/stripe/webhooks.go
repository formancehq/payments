...
type TranslateWebhookFunc func(context.Context, string, *stripe.Event) ([]connector.WebhookResponse, error)
...
func (p *Plugin) createWebhooks(ctx context.Context, req connector.CreateWebhooksRequest) ([]connector.PSPWebhookConfig, error) {
	...
	configs := make([]connector.PSPWebhookConfig, 0, len(results))
...
func (p *Plugin) extractWebhookEvent(config *connector.WebhookConfig, wh connector.PSPWebhook) (evt stripe.Event, err error) {
	if config == nil || config.Metadata == nil {
		return evt, fmt.Errorf("config metadata missing for this webhook: %w", connector.ErrWebhookVerification)
	}

	secret, ok := config.Metadata["secret"]
	if !ok {
		return evt, fmt.Errorf("secret missing from config: %w", connector.ErrWebhookVerification)
	}

	payload := wh.Body
	headers, ok := wh.Headers["Stripe-Signature"]
	if !ok || len(headers) != 1 {
		return evt, fmt.Errorf("stripe signature header not found: %w", connector.ErrWebhookVerification)
	}
	...
	if err != nil {
		return evt, fmt.Errorf("error verifying webhook signature: %w: %w", err, connector.ErrWebhookVerification)
	}
	return event, nil
}

func (p *Plugin) verifyWebhook(ctx context.Context, req connector.VerifyWebhookRequest) (*string, error) {
...
func (p *Plugin) translateWebhook(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
	event, err := p.extractWebhookEvent(req.Config, req.Webhook)
	if err != nil {
		return []connector.WebhookResponse{}, fmt.Errorf("error translating webhook: %w", err)
	}
	...
	if accountReference == "" {
		...
		if !ok {
			return []connector.WebhookResponse{}, fmt.Errorf("webhook config %q did not contain root account ID for handling Stripe Connect eventID=%s", req.Config.Name, event.ID)
		}
	...
	if !ok {
		return []connector.WebhookResponse{}, fmt.Errorf("unhandled event type: %q", event.Type)
	}
...
func translateBalanceWebhook(
	...
	accountRef string,
	evt *stripe.Event,
) ([]connector.WebhookResponse, error) {
	var balance stripe.Balance
	err := json.Unmarshal(evt.Data.Raw, &balance)
	if err != nil {
		return []connector.WebhookResponse{}, fmt.Errorf("failed to parse %q webhook JSON: %w", evt.Type, err)
	}
	responses := make([]connector.WebhookResponse, 0, len(balance.Available))
