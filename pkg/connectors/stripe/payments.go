...
func (p *Plugin) fetchNextPayments(ctx context.Context, req connector.FetchNextPaymentsRequest) (connector.FetchNextPaymentsResponse, error) {
	...
	for _, rawPayment := range rawPayments {
		...
		if err != nil {
			return connector.FetchNextPaymentsResponse{}, fmt.Errorf("failed to translate payment: %w", err)
		}
...
func (p *Plugin) translatePayment(accountRef *string, balanceTransaction *stripesdk.BalanceTransaction) (payment *connector.PSPPayment, err error) {
	...
	case stripesdk.BalanceTransactionTypePayout:
		...
		payment = &connector.PSPPayment{
			...
			Asset:     currency.FormatAsset(supportedCurrenciesWithDecimal, transactionCurrency),
			Scheme: func() connector.PaymentScheme {
	...
	case stripesdk.BalanceTransactionTypeAdjustment:
		...
		var amount int64
		var parentReference string
		var scheme connector.PaymentScheme
		var paymentStatus connector.PaymentStatus
		...
		paymentStatus = connector.PAYMENT_STATUS_DISPUTE
		switch disputeStatus {
		case connector.PAYMENT_STATUS_DISPUTE_WON:
			paymentStatus = connector.PAYMENT_STATUS_SUCCEEDED
		case connector.PAYMENT_STATUS_DISPUTE_LOST:
			paymentStatus = connector.PAYMENT_STATUS_FAILED
		}
...
func convertDisputeStatus(status stripesdk.DisputeStatus) connector.PaymentStatus {
	switch status {
	case stripesdk.DisputeStatusNeedsResponse, stripesdk.DisputeStatusUnderReview:
		return connector.PAYMENT_STATUS_DISPUTE
	case stripesdk.DisputeStatusLost:
		return connector.PAYMENT_STATUS_DISPUTE_LOST
	case stripesdk.DisputeStatusWon:
		return connector.PAYMENT_STATUS_DISPUTE_WON
	default:
		return connector.PAYMENT_STATUS_DISPUTE
	}
}

func convertPayoutStatus(status stripesdk.PayoutStatus) connector.PaymentStatus {
	switch status {
...
func detailsToPaymentScheme(details *stripesdk.ChargePaymentMethodDetails) connector.PaymentScheme {
	if details == nil || details.Card == nil {
		return connector.PAYMENT_SCHEME_UNKNOWN
	}
	scheme, _ := connector.PaymentSchemeFromString(
		fmt.Sprintf("CARD_%s", strings.ToUpper(string(details.Card.Brand))),
	)
	return scheme
}

func destinationToPaymentScheme(dest *stripesdk.PayoutDestination) connector.PaymentScheme {
	...
	case stripesdk.CardBrandUnionPay:
		return connector.PAYMENT_SCHEME_CARD_UNION_PAY
