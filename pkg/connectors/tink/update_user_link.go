...
func validateUpdateUserLinkRequest(req connector.UpdateUserLinkRequest) error {
	...
	if req.Connection.ConnectionID == "" {
		return fmt.Errorf("missing connection ID: %w", connector.ErrInvalidRequest)
	}

	if req.FormanceRedirectURL == nil || *req.FormanceRedirectURL == "" {
		return fmt.Errorf("missing formanceRedirectURL: %w", connector.ErrInvalidRequest)
	}

	if req.CallBackState == "" {
		return fmt.Errorf("missing callBackState: %w", connector.ErrInvalidRequest)
	}
