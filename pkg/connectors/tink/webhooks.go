...
func (p *Plugin) createWebhooks(ctx context.Context, req connector.CreateWebhooksRequest) (connector.CreateWebhooksResponse, error) {
	if req.ConnectorID == "" {
		return connector.CreateWebhooksResponse{}, fmt.Errorf("missing connector ID: %w", connector.ErrInvalidRequest)
	}
	if req.WebhookBaseUrl == "" {
		return connector.CreateWebhooksResponse{}, fmt.Errorf("missing webhook base URL: %w", connector.ErrInvalidRequest)
	}
	...
	for eventType, w := range p.supportedWebhooks {
		...
		if err != nil {
			return connector.CreateWebhooksResponse{}, fmt.Errorf("failed to join path: %w", err)
		}

		resp, err := p.client.CreateWebhook(ctx, eventType, req.ConnectorID, webhookURL)
		if err != nil {
			return connector.CreateWebhooksResponse{}, fmt.Errorf("failed to create webhook: %w", err)
		}
...
func (p *Plugin) verifyWebhook(_ context.Context, req connector.VerifyWebhookRequest) (connector.VerifyWebhookResponse, error) {
	if req.Config == nil {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("missing webhook config: %w", connector.ErrWebhookVerification)
	}

	if req.Webhook.Headers == nil {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("missing webhook: %w", connector.ErrWebhookVerification)
	}

	header, ok := req.Webhook.Headers["X-Tink-Signature"]
	if !ok || len(header) != 1 {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("missing tink signature header: %w", connector.ErrWebhookVerification)
	}

	timestamp, signature, err := splitVerificationHeader(header[0])
	if err != nil {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("invalid tink signature header: %w", connector.ErrWebhookVerification)
	}

	timestampInt, err := strconv.ParseInt(timestamp, 10, 64)
	if err != nil {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("failed to parse timestamp: %w", err)
	}

	createdAt := time.Unix(timestampInt, 0)
	if time.Since(createdAt) > webhookCreatedAtThreshold {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("webhook created at %s is too old: %w", createdAt, connector.ErrWebhookVerification)
	}
	...
	if secret == "" {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("invalid signature: %w", connector.ErrWebhookVerification)
	}
...
func splitVerificationHeader(header string) (string, string, error) {
	...
	if timestamp == "" || signature == "" {
		return "", "", fmt.Errorf("invalid tink signature header: %w", connector.ErrWebhookVerification)
	}
	return timestamp, signature, nil
}

func (p *Plugin) handleAccountBookedTransactionsModified(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
...
func (p *Plugin) handleAccountTransactionsModified(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
	...
	response := connector.WebhookResponse{
		DataReadyToFetch: &connector.PSPDataReadyToFetch{
			PSUID:       &psuID,
			FromPayload: payload,
			DataToFetch: []connector.OpenBankingDataToFetch{
				connector.OpenBankingDataToFetchAccountsAndBalances,
				connector.OpenBankingDataToFetchPayments,
			},
		},
	}

	return []connector.WebhookResponse{response}, nil
}

func (p *Plugin) handleAccountTransactionsDeleted(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
	...
	responses := make([]connector.WebhookResponse, 0, len(accountTransactionsDeletedWebhook.Transactions.IDs))
	for _, transactionID := range accountTransactionsDeletedWebhook.Transactions.IDs {
		response := connector.WebhookResponse{
			PaymentToCancel: &connector.PSPPaymentsToCancel{
...
func (p *Plugin) handleAccountCreated(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
...
func (p *Plugin) handleAccountUpdated(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
	// https://docs.tink.com/entries/articles/event-account-updated

	return nil, nil
}

func (p *Plugin) handleRefreshFinished(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
