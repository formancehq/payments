...
func (p *Plugin) createWebhooks(_ context.Context, _ connector.CreateWebhooksRequest) (connector.CreateWebhooksResponse, error) {
...
func (p *Plugin) verifyWebhook(ctx context.Context, req connector.VerifyWebhookRequest) (connector.VerifyWebhookResponse, error) {
	...
	if len(tokenStrings) != 1 {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("invalid token: %w", connector.ErrInvalidRequest)
	}
	...
	token, err := jwt.Parse(tokenStrings[0], func(token *jwt.Token) (interface{}, error) {
		...
		if !ok {
			return nil, fmt.Errorf("missing kid in token header: %w", connector.ErrInvalidRequest)
		}
		...
		if key.ExpiredAt.IsSet() {
			// reject expired keys
			return nil, fmt.Errorf("expired key: %w", connector.ErrInvalidRequest)
		}
	...
	if err != nil {
		return connector.VerifyWebhookResponse{}, fmt.Errorf("failed to verify token: %w", err)
	}
...
func (p *Plugin) handleAllWebhook(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
	...
	default:
		return []connector.WebhookResponse{}, fmt.Errorf("unsupported webhook type: %s", baseWebhook.WebhookType)
	}
}

func (p *Plugin) handleLinkWebhook(ctx context.Context, req connector.TranslateWebhookRequest, baseWebhook client.BaseWebhooks) ([]connector.WebhookResponse, error) {
...
func (p *Plugin) handleSessionFinishedWebhook(ctx context.Context, req connector.TranslateWebhookRequest) ([]connector.WebhookResponse, error) {
	...
	if !ok || len(ids) != 1 {
		return nil, fmt.Errorf("missing attemptID: %w", connector.ErrInvalidRequest)
	}

	attemptID, err := uuid.Parse(ids[0])
	if err != nil {
		return nil, fmt.Errorf("invalid attemptID: %w", connector.ErrInvalidRequest)
	}

	status := connector.OpenBankingConnectionAttemptStatusPending
	var errMsg *string
	switch strings.ToLower(webhook.GetStatus()) {
	case "success":
		status = connector.OpenBankingConnectionAttemptStatusCompleted
	case "exited":
		errMsg = pointer.For("exited")
		status = connector.OpenBankingConnectionAttemptStatusExited
	}
	...
	return []connector.WebhookResponse{
		{
			UserLinkSessionFinished: &connector.PSPUserLinkSessionFinished{
				AttemptID: attemptID,
				Status:    status,
				Error:     errMsg,
			},
...
func (p *Plugin) handleItemWebhook(req connector.TranslateWebhookRequest, baseWebhook client.BaseWebhooks) ([]connector.WebhookResponse, error) {
	switch baseWebhook.WebhookCode {
	case "ERROR":
	...
	case "PENDING_DISCONNECT":
		...
		return []connector.WebhookResponse{
			{
				UserConnectionPendingDisconnect: &connector.PSPUserConnectionPendingDisconnect{
...
func (p *Plugin) handleTransactionsWebhook(req connector.TranslateWebhookRequest, baseWebhook client.BaseWebhooks) ([]connector.WebhookResponse, error) {
	switch baseWebhook.WebhookCode {
