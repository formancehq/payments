...
func TestTransactionToPayment(t *testing.T) {
	...
	t.Run("other payment type - deleted status", func(t *testing.T) {
		...
		expected := connector.PSPPayment{
			...
			Reference:       transaction.ID,
			CreatedAt:       now,
			Type:            connector.PAYMENT_TYPE_OTHER,
			Amount:          big.NewInt(10000),
			Asset:           "EUR/2",
			Scheme:          connector.PAYMENT_SCHEME_OTHER,
...
func comparePSPPayments(t *testing.T, a, b connector.PSPPayment) {
...
func TestMatchTransactionType(t *testing.T) {
	tests := []struct {
		...
		expectedPaymentType connector.PaymentType
	}{
		{
			entityType:          "inbound_funds",
			transactionType:     "credit",
			expectedPaymentType: connector.PAYMENT_TYPE_PAYIN,
		},
		{
			entityType:          "payment",
			transactionType:     "debit",
			expectedPaymentType: connector.PAYMENT_TYPE_PAYOUT,
		},
		{
			entityType:          "transfer",
			transactionType:     "debit",
			expectedPaymentType: connector.PAYMENT_TYPE_TRANSFER,
		},
		{
		...
		},
		{
			...
			expectedPaymentType: connector.PAYMENT_TYPE_OTHER,
		},
		{
			entityType:          "unknown",
			transactionType:     "credit",
