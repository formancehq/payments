...
import (
	...
	"github.com/formancehq/payments/pkg/connectors/ee/fireblocks/client"
...
var _ = Describe("Fireblocks Plugin Payments", func() {
	...
	It("classifies PAY-IN when source is external", func(ctx SpecContext) {
		...
		resp, err := plg.FetchNextPayments(ctx, connector.FetchNextPaymentsRequest{PageSize: 1})
		Expect(err).To(BeNil())
	...
	It("classifies OTHER for non-transfer operations", func(ctx SpecContext) {
		...
		Expect(err).To(BeNil())
		Expect(resp.Payments).To(HaveLen(1))
		Expect(resp.Payments[0].Type).To(Equal(connector.PaymentType(connector.PAYMENT_TYPE_OTHER)))
	})
