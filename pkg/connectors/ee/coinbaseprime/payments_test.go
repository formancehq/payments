...
var _ = Describe("Coinbase Plugin Payments", func() {
	...
	Context("fetching next payments", func() {
		...
		It("should fetch payments successfully", func(ctx SpecContext) {
			...
			Expect(resp.Payments[0].Type).To(Equal(connector.PAYMENT_TYPE_PAYIN))
			Expect(resp.Payments[0].Status).To(Equal(connector.PAYMENT_STATUS_SUCCEEDED))
			...
			Expect(resp.Payments[1].Type).To(Equal(connector.PAYMENT_TYPE_PAYOUT))
			Expect(resp.Payments[1].Status).To(Equal(connector.PAYMENT_STATUS_PENDING))
			...
			Expect(resp.Payments[2].Type).To(Equal(connector.PAYMENT_TYPE_TRANSFER))
			Expect(resp.Payments[2].Status).To(Equal(connector.PAYMENT_STATUS_SUCCEEDED))
			...
			Expect(resp.Payments[3].Type).To(Equal(connector.PAYMENT_TYPE_PAYIN))
			Expect(resp.Payments[3].Status).To(Equal(connector.PAYMENT_STATUS_PENDING))
		...
		It("should handle failed transactions", func(ctx SpecContext) {
			...
			Expect(err).To(BeNil())
			Expect(resp.Payments).To(HaveLen(1))
			Expect(resp.Payments[0].Status).To(Equal(connector.PAYMENT_STATUS_FAILED))
		})
		...
		It("should handle cancelled transactions", func(ctx SpecContext) {
			...
			Expect(err).To(BeNil())
			Expect(resp.Payments).To(HaveLen(1))
			Expect(resp.Payments[0].Status).To(Equal(connector.PAYMENT_STATUS_CANCELLED))
		})
		...
		It("should handle expired transactions", func(ctx SpecContext) {
			...
			Expect(err).To(BeNil())
			Expect(resp.Payments).To(HaveLen(1))
			Expect(resp.Payments[0].Status).To(Equal(connector.PAYMENT_STATUS_EXPIRED))
		})
		...
		It("should handle retried transactions as unknown", func(ctx SpecContext) {
			...
			Expect(err).To(BeNil())
			Expect(resp.Payments).To(HaveLen(1))
			Expect(resp.Payments[0].Status).To(Equal(connector.PAYMENT_STATUS_UNKNOWN))
		})
		...
		It("should handle UNSTAKE as payout", func(ctx SpecContext) {
			...
			Expect(err).To(BeNil())
			Expect(resp.Payments).To(HaveLen(1))
			Expect(resp.Payments[0].Type).To(Equal(connector.PAYMENT_TYPE_PAYOUT))
		})
		...
		It("should handle DELEGATION as transfer", func(ctx SpecContext) {
			...
			Expect(err).To(BeNil())
			Expect(resp.Payments).To(HaveLen(1))
			Expect(resp.Payments[0].Type).To(Equal(connector.PAYMENT_TYPE_TRANSFER))
		})
