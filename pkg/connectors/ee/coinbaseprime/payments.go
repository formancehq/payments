...
func (p *Plugin) fetchNextPayments(ctx context.Context, req connector.FetchNextPaymentsRequest) (connector.FetchNextPaymentsResponse, error) {
	...
	payments := make([]connector.PSPPayment, 0, len(response.Transactions))
	for _, tx := range response.Transactions {
		payment, err := p.transactionToPayment(tx)
		if err != nil {
			return connector.FetchNextPaymentsResponse{}, fmt.Errorf("failed to convert transaction %s: %w", tx.ID, err)
		}
		if payment == nil {
			continue
		}
...
func (p *Plugin) transactionToPayment(tx client.Transaction) (*connector.PSPPayment, error) {
...
func transactionTypeToPaymentType(txType string) connector.PaymentType {
...
func transactionStatusToPaymentStatus(status string) connector.PaymentStatus {
