...
type Client interface {
	...
	PostV1CounterParties(ctx context.Context, newExternalBankAccount connector.BankAccount) (*counterparties.PostV1CounterpartiesCreated, error)
...
func New(name string, baseURL *url.URL, accessKey, secret string) Client {
	return &client{
		...
		httpClient: metrics.NewHTTPClient(name, connector.DefaultConnectorClientTimeout),
	}
}
