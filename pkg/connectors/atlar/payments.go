...
func determinePaymentType(item *atlar_models.Transaction) connector.PaymentType {
...
func determinePaymentStatus(item *atlar_models.Transaction) connector.PaymentStatus {
...
func determinePaymentScheme(item *atlar_models.Transaction) connector.PaymentScheme {
	// item.Characteristics.BankTransactionCode.Domain
	// item.Characteristics.BankTransactionCode.Family
	// TODO: fees and interest -> connector.PaymentSchemeOther with additional info on metadata. Will need example transactions for that
	...
	return connector.PAYMENT_SCHEME_SEPA
}
