...
var _ = Describe("Modulr Plugin Transaction to Payments", func() {
	...
	Context("transaction to payments", func() {
		...
		It("should return an error - wrong amount string", func(ctx SpecContext) {
			...
			po.Amount = "wrong"
			payment, err := plg.transactionToPayment(ctx, po, connector.PSPAccount{})
			Expect(err).ToNot(BeNil())
		...
		It("should return a payin payment", func(ctx SpecContext) {
			payment, err := plg.transactionToPayment(ctx, samplePayinTransaction, connector.PSPAccount{Reference: "acc1"})
			Expect(err).To(BeNil())
			Expect(payment).ToNot(BeNil())

			expected := connector.PSPPayment{
		...
		It("should return a payout payment", func(ctx SpecContext) {
			payment, err := plg.transactionToPayment(ctx, samplePayoutTransaction, connector.PSPAccount{Reference: "acc2"})
			Expect(err).To(BeNil())
			Expect(payment).ToNot(BeNil())
		...
		It("should return a transfer payment", func(ctx SpecContext) {
			...
			payment, err := plg.transactionToPayment(ctx, sampleTransferTransaction, connector.PSPAccount{Reference: "acc1"})
			Expect(err).To(BeNil())
			Expect(payment).ToNot(BeNil())
		...
		It("should return a nil payment - transfer in debit", func(ctx SpecContext) {
			...
			payment, err := plg.transactionToPayment(ctx, tr, connector.PSPAccount{Reference: "acc1"})
			Expect(err).To(BeNil())
			Expect(payment).To(BeNil())
		})
	})
...
func comparePSPPayments(a, b connector.PSPPayment) {
