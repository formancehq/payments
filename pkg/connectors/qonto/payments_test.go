...
var _ = Describe("Qonto *Plugin Payments", func() {
	Context("fetch next payments", func() {
		...
		BeforeEach(func() {
			...
			pageSize = 50
			from, _ = json.Marshal(connector.PSPAccount{Reference: "bankAccountId"})
		})
		...
		Describe("transaction to payment mapping", func() {
			...
			It("simple transaction", func(ctx SpecContext) {
				...
				Expect(resp.Payments[0].Scheme).To(Equal(connector.PAYMENT_SCHEME_UNKNOWN))
			...
			It("should map a direct debit transaction to a payment with dest account", func(ctx SpecContext) {
				...
				Expect(resp.Payments[0].Type).To(Equal(connector.PAYMENT_TYPE_PAYOUT))
				Expect(resp.Payments[0].Scheme).To(Equal(connector.PAYMENT_SCHEME_SEPA_DEBIT))
				Expect(resp.Payments[0].DestinationAccountReference).To(Equal(pointer.For("IBAN-BIC")))
			})
			It("should map a direct debit collection transaction to a payment with dest account", func(ctx SpecContext) {
				...
				Expect(resp.Payments[0].Scheme).To(Equal(connector.PAYMENT_SCHEME_SEPA_CREDIT))
				Expect(resp.Payments[0].DestinationAccountReference).To(Equal(pointer.For("IBAN-BIC")))
			})
			...
			It("should map a pending transaction to the right status", func(ctx SpecContext) {
				...
				Expect(resp.Payments[0].Status).To(Equal(connector.PAYMENT_STATUS_PENDING))
			})
...
func assertTransactionsErrorResponse(resp connector.FetchNextPaymentsResponse, err error, expectedError error) {
...
func assertTransactionsSuccessResponse(
	resp connector.FetchNextPaymentsResponse,
	err error,
...
func assertSimpleTransactionsMapping(transaction client.Transactions, resultingPSPAPayment connector.PSPPayment) {
