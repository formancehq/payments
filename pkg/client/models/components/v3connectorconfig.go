...
const (
	...
	V3ConnectorConfigTypeCoinbase      V3ConnectorConfigType = "Coinbase"
...
func CreateV3ConnectorConfigCoinbase(coinbase V3CoinbaseConfig) V3ConnectorConfig {
	typ := V3ConnectorConfigTypeCoinbase
...
func (u *V3ConnectorConfig) UnmarshalJSON(data []byte) error {
	...
	case "Coinbase":
		...
		if err := utils.UnmarshalJSON(data, &v3CoinbaseConfig, "", true, false); err != nil {
			return fmt.Errorf("could not unmarshal `%s` into expected (Provider == Coinbase) type V3CoinbaseConfig within V3ConnectorConfig: %w", string(data), err)
		}

		u.V3CoinbaseConfig = v3CoinbaseConfig
		u.Type = V3ConnectorConfigTypeCoinbase
		return nil
	case "Column":
