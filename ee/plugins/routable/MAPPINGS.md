...
| `payoutsPerMinute` | no | `90` | Ceiling on payout and transfer initiations sent to Routable, in the same per-minute unit as the budget in [§6.1](#61-routable-rps-budget). Unset (absent, `null` or `0`) means 90/min. Whole and unsigned: negative and fractional values are refused at install/update. |
