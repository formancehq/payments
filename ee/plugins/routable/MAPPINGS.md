...
| `payoutsPerMinute` | no | `90` | Ceiling on payout and transfer initiations sent to Routable, in the same per-minute unit as the budget in [§6.1](#61-routable-rps-budget) and Routable's own 60s rate-limit window ([§6.1.1](#611-rate-limit-envelope-informational)). Raise it when the account is provisioned above the documented limits. Left unset — absent, `null` or `0`, which is how every config written before this field existed reads — [`plugin.go`](plugin.go) applies `DefaultPayoutsPerMinute` (90/min = the 1.5/s this connector ran at before the field was configurable), so installed connectors are unaffected. `Plugin.PayoutsPerSecond` divides it by 60 for the engine's per-second throttle (`models.PluginWithPayoutThrottle`); the rate it resolves is always > 0, which is what keeps the connector's dedicated payout task queue alive. Whole and unsigned, so negative and fractional values are refused at install/update rather than truncated. |
