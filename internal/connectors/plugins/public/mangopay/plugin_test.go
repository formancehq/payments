...
var _ = Describe("Mangopay Plugin", func() {
	...
	Context("install", func() {
		...
		It("should report errors in config - apiKey", func(ctx SpecContext) {
			req := models.InstallRequest{Config: json.RawMessage(`{"clientID": "test", "endpoint": "test"}`)}
			_, err := plg.Install(ctx, req)
		...
		It("should report errors in config - endpoint", func(ctx SpecContext) {
			req := models.InstallRequest{Config: json.RawMessage(`{"clientID": "test", "apiKey": "test"}`)}
			_, err := plg.Install(ctx, req)
		...
		It("should return valid install response", func(ctx SpecContext) {
			req := models.InstallRequest{Config: json.RawMessage(`{"clientID": "test", "apiKey": "test", "endpoint": "test"}`)}
	...
	Context("reverse transfer", func() {
		It("should fail because unimplemented", func(ctx SpecContext) {
