...
var _ = Describe("Generic Plugin Accounts", func() {
	...
	Context("fetching next accounts", func() {
		...
		It("should return an error - get accounts error", func(ctx SpecContext) {
			...
			m.EXPECT().ListAccounts(ctx, int64(0), int64(60), time.Time{}).Return(
		...
		It("should fetch next accounts - no state pageSize < total accounts", func(ctx SpecContext) {
			...
			m.EXPECT().ListAccounts(ctx, int64(0), int64(40), time.Time{}).Return(
				sampleAccounts[:40],
				nil,
			)
		...
		It("should fetch next accounts - with state pageSize < total accounts", func(ctx SpecContext) {
			...
			m.EXPECT().ListAccounts(ctx, int64(0), int64(40), sampleAccounts[38].CreatedAt.UTC()).Return(
				sampleAccounts[:40],
				nil,
			)

			m.EXPECT().ListAccounts(ctx, int64(1), int64(40), sampleAccounts[38].CreatedAt.UTC()).Return(
				sampleAccounts[40:],
				nil,
			)
