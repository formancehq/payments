...
func fillParentReference(transaction *client.Transaction, pspPayment models.PSPPayment) models.PSPPayment {
	if transaction.Source.CheckDepositID != "" {
		pspPayment.ParentReference = transaction.Source.CheckDepositID
	} else if transaction.Source.InboundCheckDepositID != "" {
		pspPayment.ParentReference = transaction.Source.InboundCheckDepositID
	} else if transaction.Source.InboundAchTransferID != "" {
		pspPayment.ParentReference = transaction.Source.InboundAchTransferID
	} else if transaction.Source.ID != "" {
		pspPayment.ParentReference = transaction.Source.ID
	} else if transaction.Source.TransferID != "" {
		pspPayment.ParentReference = transaction.Source.TransferID
	} else if transaction.Source.WireTransferID != "" {
		pspPayment.ParentReference = transaction.Source.WireTransferID
	}
...
func fillAccountID(transaction *client.Transaction, pspPayment models.PSPPayment) models.PSPPayment {
	...
	if category == "account_transfer_intention" {
...
func mapTransactionType(transaction *client.Transaction) models.PaymentType {
	...
	transactionAmount := transaction.Source.Amount
