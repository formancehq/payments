	...
	Context("fetching next payments", func() {
		...
		It("should fetch next payments - no empty reference when transfer_id is empty", func(ctx SpecContext) {
			...
			mockHTTPClient.EXPECT().Do(
				...
				Data:       sampleSucceededTransactions[:2],
				NextCursor: "qwerty",
			})
			...
			mockHTTPClient.EXPECT().Do(
				...
				Data:       samplePendingTransactions[:2],
				NextCursor: "zxcvb",
			})
			...
			mockHTTPClient.EXPECT().Do(
				...
				Data:       sampleDeclinedTransactions[:2],
				NextCursor: "asdfg",
			})

			resp, err := plg.FetchNextPayments(ctx, req)
			Expect(err).To(BeNil())
			Expect(resp.Payments[0].Reference).To(Equal("pa_0"))
			Expect(resp.Payments[0].ParentReference).To(Equal("account_transfer_87654"))
			Expect(resp.Payments[2].Reference).To(Equal("pa_0"))
			Expect(resp.Payments[2].ParentReference).To(Equal("check_deposit_12345"))
			Expect(resp.Payments[4].Reference).To(Equal("pa_0"))
			Expect(resp.Payments[4].ParentReference).To(Equal("check_deposit_12345"))
			Expect(resp.HasMore).To(BeTrue())
			Expect(resp.NewState).ToNot(BeNil())
