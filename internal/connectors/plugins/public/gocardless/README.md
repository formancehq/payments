...
```json
{
  "reference": "CR123", // For creditor accounts
  // OR
  "reference": "CU123" // For customer accounts
}
```
...
The connector supports fetching both creditors and customers through the `FetchNextOthers` endpoint. The type of user to fetch is determined by the reference prefix in the request payload.

#### Fetching Creditors

```json
{
  "reference": "CR123" // Must start with "CR" for creditors
}
```
...
```json
{
  "reference": "CU123" // Must start with "CU" for customers
}
```
