...
func (c *client) GetCustomers(ctx context.Context, pageSize int, after string, before string) ([]GocardlessUser, Cursor, error) {

	ctx = context.WithValue(ctx, metrics.MetricOperationContextKey, "list_customers")

	customersResponse, err := c.service.Customers.List(ctx, gocardless.CustomerListParams{
		Limit:  pageSize,
		After:  after,
		Before: before,
	})
	...
	nextCursor := Cursor{
		...
		Before: customersResponse.Meta.Cursors.Before,
	}
