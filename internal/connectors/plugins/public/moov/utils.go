...
func extractFacilitatorFee(pi models.PSPPaymentInitiation) (moov.CreateTransfer_FacilitatorFee, error) {
	facilitatorFee := models.ExtractNamespacedMetadata(pi.Metadata, client.MoovFacilitatorFeeTotalMetadataKey)
	...
	var err error
	if facilitatorFee != "" {
		facilitatorTotal, err = strconv.ParseInt(facilitatorFee, 10, 64)
		if err != nil {
	...
	return moov.CreateTransfer_FacilitatorFee{
		Total:         &facilitatorTotal,
		TotalDecimal:  &facilitatorTotalDecimal,
		Markup:        &facilitatorMarkup,
		MarkupDecimal: &facilitatorMarkupDecimal,
	}, nil
}
...
func extractSalesTax(pi models.PSPPaymentInitiation) (moov.Amount, error) {
	...
	return moov.Amount{
		Currency: salesTaxAmountCurrency,
...
func extractPaymentSource(pi models.PSPPaymentInitiation) moov.CreateTransfer_Source {
	...
	return moov.CreateTransfer_Source{
