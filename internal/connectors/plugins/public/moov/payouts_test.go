...
var _ = Describe("Moov Payouts", func() {
	...
	It("should return empty sales tax when not provided", func() {
		...
		Expect(err).To(BeNil())
		Expect(salesTax.Value).To(Equal(int64(0)))
		Expect(salesTax.Currency).To(Equal(""))
	})
	...
	It("should return empty facilitator fee when not provided", func() {
		...
		Expect(err).To(BeNil())
		Expect(*fee.Total).To(Equal(int64(0)))
		Expect(*fee.TotalDecimal).To(Equal(""))
		Expect(*fee.Markup).To(Equal(int64(0)))
		Expect(*fee.MarkupDecimal).To(Equal(""))
	})
	...
	Context("create payout with moov client", func() {
		...
		It("should return error when source payment method exceeds transaction limit", func() {
			// Create transfer options with a source payment method that has a limit
			transferOptions := &moov.TransferOptions{
				SourceOptions: []moov.PaymentMethod{
					{
						PaymentMethodID:   "src-payment-method-123",
						PaymentMethodType: "ach-debit-fund", // This has a limit of $99_999_999.99
					},
				},
				DestinationOptions: []moov.PaymentMethod{
					{
			...
			// Request with amount exceeding the limit for ach-debit-fund ($99_999_999.99)
			request := moov.CreateTransfer{
				Source: moov.CreateTransfer_Source{
					PaymentMethodID: "src-payment-method-123",
				},
				...
				Amount: moov.Amount{
					Currency: "USD",
					Value:    150000000000, // $150,000,000, exceeds the $99_999_999.99 limit
				},
			}
			...
			Expect(err).To(MatchError(models.NewConnectorValidationError("amount", errors.New("transaction amount 150000000000 exceeds source payment method limit of 99999999.990000 for ach-debit-fund"))))
			Expect(response).To(BeNil())
		})

		It("should return error when destination payment method exceeds transaction limit", func() {
			// Create transfer options with a destination payment method that has a limit
			transferOptions := &moov.TransferOptions{
				SourceOptions: []moov.PaymentMethod{
					{
						PaymentMethodID:   "src-payment-method-123",
				...
				DestinationOptions: []moov.PaymentMethod{
					{
						PaymentMethodID:   "dst-payment-method-456",
						PaymentMethodType: "ach-credit-same-day", // This has a limit of $500,000
					},
				},
			}

			// Request with amount exceeding the limit for ach-credit-same-day ($500,000)
			request := moov.CreateTransfer{
				Source: moov.CreateTransfer_Source{
					PaymentMethodID: "src-payment-method-123",
				},
				...
				Amount: moov.Amount{
					Currency: "USD",
					Value:    600000, // $600,000, exceeds the $500,000 limit
				},
			}
			...
			Expect(err).To(MatchError(models.NewConnectorValidationError("amount", errors.New("transaction amount 600000 exceeds destination payment method limit of 500000.000000 for ach-credit-same-day"))))
			Expect(response).To(BeNil())
		})

		It("should successfully create a payout when transaction amount is within limits", func() {
			// Create transfer options with payment methods that have limits
			transferOptions := &moov.TransferOptions{
				SourceOptions: []moov.PaymentMethod{
					{
						PaymentMethodID:   "src-payment-method-123",
						PaymentMethodType: "ach-debit-fund", // This has a limit of $10,000
					},
				},
				DestinationOptions: []moov.PaymentMethod{
					{
						PaymentMethodID:   "dst-payment-method-456",
						PaymentMethodType: "ach-credit-standard", // This has a limit of $25,000
					},
				},
			}

			// Request with amount within both limits
			request := moov.CreateTransfer{
				Source: moov.CreateTransfer_Source{
					PaymentMethodID: "src-payment-method-123",
				},
				...
				Amount: moov.Amount{
					Currency: "USD",
					Value:    5000, // $5,000, within both limits
				},
			}

			expectedTransfer := moov.Transfer{
				TransferID: "transfer123",
				Status:     moov.TransferStatus_Completed,
				Amount: moov.Amount{
					Value:    5000,
			...
			mockedService.EXPECT().CreateMoovTransfer(gomock.Any(), "test", gomock.Any()).
				Return(&expectedTransfer, nil, nil)

			response, err := plg.client.InitiatePayout(ctx, sourceAccountID, destAccountID, request)

			Expect(err).To(BeNil())
			Expect(response).NotTo(BeNil())
			Expect(response.TransferID).To(Equal("transfer123"))
			Expect(response.Amount.Value).To(Equal(int64(5000)))
		})

		It("should return error for unknown payment method type", func() {
			transferOptions := &moov.TransferOptions{
				SourceOptions: []moov.PaymentMethod{
					{
						PaymentMethodID:   "src-payment-method-123",
			...
			Expect(err).To(MatchError(models.NewConnectorValidationError("amount", errors.New("transaction amount 1000 exceeds source payment method limit of 0.000000 for unknown-payment-method"))))
			Expect(response).To(BeNil())
		})
	})
})

var _ = Describe("Moov Transaction Limit Validation", func() {
	Context("ValidateTransactionLimit", func() {
		It("should return true when amount is within the limit", func() {
			pmType := client.PaymentMethodTypeACHDebitFund // Limit: $99_999_999.99
			amount := 5000.00                              // $5,000

			isValid, limit := client.ValidateTransactionLimit(pmType, amount)

			Expect(isValid).To(BeTrue())
			Expect(limit).To(Equal(99_999_999.99))
		})

		It("should return true when amount equals the limit", func() {
			pmType := client.PaymentMethodTypeACHCreditStandard // Limit: $99_999_999.99
			amount := 99_999_999.99                             // $25,000
		...
		It("should return false when amount exceeds the limit", func() {
			pmType := client.PaymentMethodTypeACHCreditSameDay // Limit: $500,000
			amount := 600000.00                                // $600,000

			isValid, limit := client.ValidateTransactionLimit(pmType, amount)

			Expect(isValid).To(BeFalse())
			Expect(limit).To(Equal(500000.0))
		})

		It("should return false for unknown payment method type", func() {
			pmType := client.PaymentMethodType("unknown-type")
			amount := 100.00

			isValid, limit := client.ValidateTransactionLimit(pmType, amount)

			Expect(isValid).To(BeFalse())
			Expect(limit).To(Equal(0.0))
		})

		It("should handle large transaction amounts", func() {
			pmType := client.PaymentMethodTypeACHDebitFund // Limit: $99,999,999.99
			amount := 50000000.00                          // $50,000,000

			isValid, limit := client.ValidateTransactionLimit(pmType, amount)

			Expect(isValid).To(BeTrue())
			Expect(limit).To(Equal(99999999.99))
		})
