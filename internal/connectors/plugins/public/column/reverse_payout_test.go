	...
	Context("fetching next payments", func() {
		...
		Context("validateReversePayout", func() {
			...
			It("should return error when reason is missing", func() {
				...
				err := plg.validateReversePayout(pr)
				Expect(err).To(MatchError(fmt.Sprintf("required field metadata field %s must be provided", client.ColumnReasonMetadataKey)))
			})
			...
			It("should return error when reason is invalid", func() {
				...
				err := plg.validateReversePayout(pr)
				Expect(err).To(MatchError(fmt.Sprintf("required field metadata field %s must be a valid reason", client.ColumnReasonMetadataKey)))
			})
		})
