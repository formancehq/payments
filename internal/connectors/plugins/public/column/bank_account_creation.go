...
func (c *Plugin) createBankAccount(ctx context.Context, ba models.BankAccount) (models.CreateBankAccountResponse, error) {
	...
	city := models.ExtractNamespacedMetadata(ba.Metadata, client.ColumnCityMetadataKey)
	state := models.ExtractNamespacedMetadata(ba.Metadata, client.ColumnStateMetadataKey)
	postalCode := models.ExtractNamespacedMetadata(ba.Metadata, client.ColumnPostalCodeMetadataKey)
	...
	bankAccount, err := c.client.CreateCounterPartyBankAccount(ctx, data)
