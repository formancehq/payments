...
func (p *Plugin) fillPayments(
	...
	for _, transaction := range pagedTransactions {
		...
		precision, ok := supportedCurrenciesWithDecimal[transaction.CurrencyCode]
		if !ok {
			return nil, fmt.Errorf("unsupported currency: %s", transaction.CurrencyCode)
		}

		amt := strconv.Itoa(int(transaction.Amount))
		amount, err := currency.GetAmountWithPrecisionFromString(amt, precision)
		if err != nil {
			return nil, fmt.Errorf("failed to parse amount %s: %w", amt, err)
		}
