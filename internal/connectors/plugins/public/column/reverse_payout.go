...
func (p *Plugin) createReversePayout(ctx context.Context, pr models.PSPPaymentInitiationReversal) (models.ReversePayoutResponse, error) {
	...
	payment, err := transformReversePayoutResponse(resp)
	if err != nil {
		return models.ReversePayoutResponse{}, err
	}
...
func transformReversePayoutResponse(resp *client.ReversePayoutResponse) (models.PSPPayment, error) {
	...
	return models.PSPPayment{
		...
		Status:                      matchStatus(resp.Status),
		Raw:                         raw,
		Reference:                   resp.ID,
		CreatedAt:                   createdAt,
