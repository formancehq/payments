...
func (p *Plugin) fetchQuote(ctx context.Context, req models.GetQuoteRequest) (models.GetQuoteResponse, error) {
	...
	market := convertPairForBitstamp(req.SourceAsset + req.TargetAsset)

	// Bitstamp doesn't have a dedicated quote endpoint, so we use the order book
	...
	// Estimate fee (Bitstamp has tiered fees, use a conservative 0.5% estimate)
	// Fee = (price * quantity) * 0.005
	if req.Quantity != nil {
		totalCost := new(big.Int).Mul(price, req.Quantity)
		fee = new(big.Int).Div(totalCost, big.NewInt(200)) // 0.5% = 1/200
	} else {
		fee = big.NewInt(0)
	}
