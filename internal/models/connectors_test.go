...
func TestConnectorIDOnly_MarshalJSON(t *testing.T) {
	t.Parallel()
	...
	in := models.ConnectorIDOnly{ID: id}
	...
	// Only the id field should be present and should contain the encoded ID string
	assert.Equal(t, 1, len(m))
	assert.Equal(t, id.String(), m["id"])

	// Zero-value ID should marshal to empty string
	zero := models.ConnectorIDOnly{}
...
func TestConnectorIDOnly_UnmarshalJSON(t *testing.T) {
	t.Parallel()

	validID := models.ConnectorID{
		Provider:  "test",
	...
	t.Run("valid id", func(t *testing.T) {
		t.Parallel()
		var out models.ConnectorIDOnly
		err := json.Unmarshal([]byte(`{"id":"`+encoded+`"}`), &out)
		require.NoError(t, err)
		assert.Equal(t, validID.Provider, out.ID.Provider)
		assert.Equal(t, validID.Reference, out.ID.Reference)
		assert.Equal(t, encoded, out.ID.String())
	})

	t.Run("invalid id string", func(t *testing.T) {
		t.Parallel()
		var out models.ConnectorIDOnly
		err := json.Unmarshal([]byte(`{"id":"not-base64"}`), &out)
		require.Error(t, err)
	})
