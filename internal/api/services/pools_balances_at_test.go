...
func TestPoolsBalancesHistorical(t *testing.T) {
	t.Parallel()
...
func TestPoolsBalancesLatest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.poolsGetStorageErr == nil {
				store.EXPECT().BalancesGetLatest(gomock.Any(), models.AccountID{}).Return(balancesResponse, test.accountsBalancesAtErr)
			}
