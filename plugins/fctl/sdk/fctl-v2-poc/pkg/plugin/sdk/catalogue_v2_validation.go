...
func validateRequestSigning(command Command) error {
	...
	if command.ExecutionKind != ExecutionKindService || signing.Capability != CapabilitySignLedgerApplyBatch || signing.ProductMajor != 3 || signing.PayloadType != "formance.ledger.v3.ApplyBatch" {
		return fmt.Errorf("request signing contract is invalid")
	}
	if len(command.Compatibility) != 1 || command.Compatibility[0].Service != ServiceLedger || len(command.Compatibility[0].Majors) != 1 || command.Compatibility[0].Majors[0] != 3 {
		return fmt.Errorf("request signing requires Ledger major 3")
	}
	applyOperations := 0
	for _, operation := range command.Operations {
		if operation.GRPC == nil || operation.GRPC.FullMethod != "/ledger.BucketService/Apply" {
			continue
		}
		if operation.Service != ServiceLedger {
			return fmt.Errorf("request signing requires Ledger Apply")
		}
		applyOperations++
	}
	if applyOperations != 1 {
		return fmt.Errorf("request signing requires Ledger Apply")
	}
	return nil
}
