...
func commandToSDK(command *CommandDescriptor) (sdk.Command, error) {
	...
	if signing := command.GetRequestSigning(); signing != nil {
		mapped.RequestSigning = &sdk.RequestSigningSpec{Capability: signing.GetCapability(), ProductMajor: signing.GetProductMajor(), PayloadType: signing.GetPayloadType()}
	}
...
func commandFromSDK(command sdk.Command) (*CommandDescriptor, error) {
	...
	if command.RequestSigning != nil {
		mapped.RequestSigning = &RequestSigningSpec{Capability: command.RequestSigning.Capability, ProductMajor: command.RequestSigning.ProductMajor, PayloadType: command.RequestSigning.PayloadType}
	}
